    ...
    <div class="tree-head">
      <table :class="{isTableLayout: (list[0].planItemTimeDtoList.length>=5 ? true : false)}">
        <tr>
          ...
          <td class="td3">合计</td>
          <td class="td4" v-if="list[0]" v-for="(timeItem, index) in list[0].planItemTimeDtoList">
            {{timeItem.planCycle}}
          </td>
        </tr>
      </table>
...
//        console.log('处理前的:', JSON.parse(JSON.stringify(this.list)))
        ...
        let reduceDataFunc = (data, level) => {
          data.map((m, i) => {
            m.count = ++count;
            m.isExpand = false
            m.planItemDtoList = m.planItemDtoList || []
            m.level = level
            m.bLeft = level === 1 ? 34 : (level - 2) * 16 + 34
            m.Experience = m.Experience || '无'
            if (m.planItemDtoList.length > 0) {
              reduceDataFunc(m.planItemDtoList, level + 1)
            }
          })
        }
